                    ...
                    if (unit instanceof JInvokeStmt) {
                        ...
                        if (invokeExpr instanceof JSpecialInvokeExpr) {
                            ...
                            if (isRelevantInit(specialInvokeExpr)) {
                                int trolley_size = ((IntConstant) specialInvokeExpr.getArg(0)).value; //Maybe crushes if not int constant
                                int reserve_size = ((IntConstant) specialInvokeExpr.getArg(1)).value; // here too
                                for (Node node : getAllocationNodes(specialInvokeExpr)) {
                                    StoreInitializer initializer = new StoreInitializer((JInvokeStmt) unit, uniqueNumber, trolley_size, reserve_size);
